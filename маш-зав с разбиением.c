...
//                if (!bullets[bIdx].active || bullets[bIdx].dy <= 0.0f) continue; // только пули игрока (dy > 0)
...
//{
//    for (int i = 0; i < MAX_BULLETS; i++) {
//        if (!bullets[i].active || bullets[i].dy >= 0.0f) continue;
...
//    for (int i = 0; i < MAX_BULLETS; i++) {
//        if (!bullets[i].active || bullets[i].dy >= 0.0f) continue; // пули игрока: dy > 0, врагов: dy < 0
...
//            updateEnemy(); // ← ИСПОЛЬЗУЕТ ПРОСТРАНСТВЕННУЮ СЕТКУ
//            updateBullets();
//            for (int j = 0; j < MAX_ENEMIES; j++)
