...
#define GRID_CELL_SIZE 0.2f
#define GRID_WIDTH ((int)(2.0f / GRID_CELL_SIZE) + 1)
#define GRID_HEIGHT ((int)(2.0f / GRID_CELL_SIZE) + 1)

#if PROFILING_MODE

static double update_enemy_total_time = 0.0;
static int update_enemy_call_count = 0;
static double update_enemy_movement_total_time = 0.0;
static int update_enemy_movement_call_count = 0;
static double update_bullets_total_time = 0.0;
static int update_bullets_call_count = 0;
static double draw_enemy_total_time = 0.0;
static int draw_enemy_call_count = 0;
static double build_spatial_grid_time = 0.0;
static int build_spatial_grid_count = 0;
static double check_collisions_grid_time = 0.0;
static int check_collisions_grid_count = 0;
static FILE* opt_time_log_file = NULL;
#endif
...
void buildSpatialGrid() {
    ...
    for (int i = 0; i < MAX_BULLETS; i++) {
        if (bullets[i].active) addBulletToGrid(i);
    }
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (enemies[i].active) addEnemyToGrid(i);
    }

#if PROFILING_MODE
    clock_t end = clock();
    double cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC * 1000000.0;
    build_spatial_grid_time += cpu_time_used;
    build_spatial_grid_count++;
#endif
}
...
void checkCollisionsWithGrid() {
    ...
    for (int gx = 0; gx < GRID_WIDTH; gx++) {
        for (int gy = 0; gy < GRID_HEIGHT; gy++) {
            GridCell* cell = &spatialGrid[gx][gy];
            for (int bi = 0; bi < cell->bulletCount; bi++) {
                int bIdx = cell->bulletIndices[bi];

                if (!bullets[bIdx].active || bullets[bIdx].dy <= 0.0f) continue; //только пули игрока

                for (int ei = 0; ei < cell->enemyCount; ei++) {
                    int eIdx = cell->enemyIndices[ei];
                    if (!enemies[eIdx].active) continue;
#if PROFILING_MODE
                    collision_checks++;
#endif
                    float dx = fabsf(bullets[bIdx].x - enemies[eIdx].x);
                    float dy = fabsf(bullets[bIdx].y - enemies[eIdx].y);
                    if (dx <= ENEMY_SIZEX && dy <= ENEMY_SIZEY) {
#if PROFILING_MODE
                        collisions_detected++;
#endif
                        enemies[eIdx].lives--;
                        bullets[bIdx].active = 0;
                        bulletCount--;
                        enemies[eIdx].hit = 1;
                        if (enemies[eIdx].lives == 0) {
    ...
    clock_t end = clock();
    double cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC * 1000000.0;
    check_collisions_grid_time += cpu_time_used;
    check_collisions_grid_count++;

    static int grid_collision_counter = 0;
    grid_collision_counter++;

    if (grid_collision_counter % 60 == 0 && check_collisions_grid_count > 0) {
        double avg_time = check_collisions_grid_time / check_collisions_grid_count;


        if (opt_time_log_file) {
            fprintf(opt_time_log_file, "checkCollisionsWithGrid: %.2f median time, checks = %d, collisions = %d\n",
                avg_time, collision_checks, collisions_detected);
            fflush(opt_time_log_file);
        }

        check_collisions_grid_time = 0.0;
        check_collisions_grid_count = 0;
    }
#endif
}
...
void updateBullets()
{
    ...
    for (int i = 0; i < MAX_BULLETS; i++) {
        ...
        //апдейт всех пулек любого направления
        bullets[i].y += bullets[i].dy;

        //проверка на выход за границы
        if (bullets[i].y < -1.0f || bullets[i].y > 1.0f) {
            bullets[i].active = 0;
            bulletCount--;
        }
    }
    ...
    clock_t end = clock();
    double cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC * 1000000.0;
    update_bullets_total_time += cpu_time_used;
    update_bullets_call_count++;
#endif
}
...
void updateEnemy() {
    ...
    double total_cpu_time = ((double)(total_end - total_start)) / CLOCKS_PER_SEC * 1000000.0;

    update_enemy_total_time += total_cpu_time;
    update_enemy_call_count++;

    static int update_counter = 0;
    update_counter++;

    if (update_counter % 60 == 0 && update_enemy_call_count > 0) {
        double avg_time = update_enemy_total_time / update_enemy_call_count;

        //считаем активные объекты
        int active_enemies = 0;
        for (int i = 0; i < MAX_ENEMIES; i++) if (enemies[i].active) active_enemies++;


        if (opt_time_log_file) {
            fprintf(opt_time_log_file, "updateEnemy : time = %.2f , enemy = %d, bullets = %d\n",
                avg_time, active_enemies, bulletCount);
            fflush(opt_time_log_file);
        }

        update_enemy_total_time = 0.0;
        update_enemy_call_count = 0;
    }
#endif
}
...
void updateEnemyMovement(float playerX) {
    ...
    update_enemy_movement_total_time += cpu_time_used;
    update_enemy_movement_call_count++;

    static int movement_counter = 0;
    movement_counter++;

    if (movement_counter % 60 == 0 && update_enemy_movement_call_count > 0) {
        double avg_time = update_enemy_movement_total_time / update_enemy_movement_call_count;

        //if (opt_time_log_file) {
        //    fprintf(opt_time_log_file, "updateEnemyMovement: %.2f us (avg over %d calls)\n",
        //        avg_time, update_enemy_movement_call_count);
        //    fflush(opt_time_log_file);
        //}

        update_enemy_movement_total_time = 0.0;
        update_enemy_movement_call_count = 0;
    }
#endif
}

void diveAttack(float px) {
    if ((glfwGetTime() - lastDiveTime) < diveIntervalVar) return;
...
void checkDiveCollisions(float playerX) {
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (enemies[i].active && enemies[i].diving) {
            ...
            if ((fabsf(dx) <= PLAYER_COLLIDE_RX + ENEMY_SIZEX) &&
                (fabsf(dy) <= PLAYER_COLLIDE_RY + ENEMY_SIZEY)) {
                playerHits++;
                playerIsHit = 1;
                enemies[i].lives = 0;
                enemies[i].active = 0;
...
void updatePlayerHits(float px) {
    for (int i = 0; i < MAX_BULLETS; i++) {
        if (!bullets[i].active || bullets[i].dy >= 0.0f) continue; //1 - пули игрока 2 - врагов
...
void drawEnemiesInstanced(unsigned int texture, mat4 model, mat4 view, mat4 projection) {
    ...
    static char instanceData[MAX_ENEMIES * 16];
    int instanceCount = 0;
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (!enemies[i].active) continue;
    ...
    glBindVertexArray(0);
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (enemies[i].hit) enemies[i].hit = 0;
    }
    ...
    draw_enemy_total_time += cpu_time_used;
    draw_enemy_call_count++;

    static int draw_counter = 0;
    draw_counter++;

    if (draw_counter % 60 == 0 && draw_enemy_call_count > 0) {
        double avg_time = draw_enemy_total_time / draw_enemy_call_count;
        ...
        if (opt_time_log_file) {
            fprintf(opt_time_log_file, "time = %.2f, checks = %d (cnt calls = %d )\n",
                avg_time, active_enemies, draw_enemy_call_count);
            fflush(opt_time_log_file);
        }

        draw_enemy_total_time = 0.0;
        draw_enemy_call_count = 0;
    }
#endif
}
...
unsigned int loadTexture(const char* path) {
    ...
    else internalFormat = dataFormat = GL_RGBA;
...
void renderMenu(GLFWwindow* window) {
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
        ...
        if (ndc_x >= -0.4f && ndc_x <= 0.4f) {
            if (ndc_y >= 0.0f && ndc_y <= 0.1f) gameState = STATE_PLAYING;
            else if (ndc_y >= -0.15f && ndc_y <= -0.05f) gameState = STATE_CONTROLS;
            else if (ndc_y >= -0.3f && ndc_y <= -0.2f) gameState = STATE_HIGHSCORES;
        }
    }
}
...
{
#if PROFILING_MODE
    // Открываем файл для записи времени оптимизированной версии
    opt_time_log_file = fopen("opt_time.txt", "w");
    if (opt_time_log_file) {
        fprintf(opt_time_log_file, "\n");
        fflush(opt_time_log_file);
    }
#endif
    ...
    glfwSwapInterval(0); // Отключён VSync для профилирования
    ...
    // === Пули ===
    float vb[] = {
        -0.0025f, -0.035f, 0, 1.0f, 0.65f, 0.0f,
         0.0025f, -0.035f, 0, 1.0f, 0.65f, 0.0f,
         0.0025f, -0.015f, 0, 1.0f, 0.65f, 0.0f,
        -0.0025f, -0.035f, 0, 1.0f, 0.65f, 0.0f,
         0.0025f, -0.015f, 0, 1.0f, 0.65f, 0.0f,
        -0.0025f, -0.015f, 0, 1.0f, 0.65f, 0.0f
    };
    ...
    for (int i = 0; i < MAX_BULLETS; i++) {
        ...
        bullets[i].dy = (rand() % 2) ? BULLETSPEED : -BULLETSPEED;
        bullets[i].active = 1;
    }
    score = 999999;
    wave = 99;
...
#if PROFILING_MODE
    profileLog = fopen("profile_optimized_with_grid.txt", "w");
    if (profileLog) {
        fprintf(profileLog, "Time_sec\tFPS\tFrameTime\tActiveEnemies\tActiveBullets\tDrawCalls\n");
        fflush(profileLog);
    }
#endif
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (profileLog && (currentTime - lastReportTime >= 1.0)) {
            ...
            fprintf(profileLog, "%.3f\t%.2f\t%.3f\t%d\t%d\t%lld\n",
                currentTime, fps, frameTimeMs, activeEnemies, activeBullets, draw_call_count);
            fflush(profileLog);
    ...
    if (opt_time_log_file) {
        ...
        if (update_enemy_call_count > 0) {
            double avg_update_enemy = update_enemy_total_time / update_enemy_call_count;
            fprintf(opt_time_log_file, "updateEnemy: %.2f\n", avg_update_enemy);
        }

        if (build_spatial_grid_count > 0) {
            double avg_build_grid = build_spatial_grid_time / build_spatial_grid_count;
            fprintf(opt_time_log_file, "buildSpatialGrid: %.2f \n", avg_build_grid);
        }

        if (check_collisions_grid_count > 0) {
            double avg_check_collisions = check_collisions_grid_time / check_collisions_grid_count;
            fprintf(opt_time_log_file, "checkCollisionsWithGrid: %.2f \n", avg_check_collisions);
        }

        if (update_enemy_movement_call_count > 0) {
            double avg_update_movement = update_enemy_movement_total_time / update_enemy_movement_call_count;
            fprintf(opt_time_log_file, "updateEnemyMovement: %.2f\n", avg_update_movement);
        }

        if (update_bullets_call_count > 0) {
            double avg_update_bullets = update_bullets_total_time / update_bullets_call_count;
            fprintf(opt_time_log_file, "updateBullets: %.2f\n", avg_update_bullets);
        }

        if (draw_enemy_call_count > 0) {
            double avg_draw_enemy = draw_enemy_total_time / draw_enemy_call_count;
            //fprintf(opt_time_log_file, " %.2f\n", avg_draw_enemy);
        }

        fclose(opt_time_log_file);
    }
