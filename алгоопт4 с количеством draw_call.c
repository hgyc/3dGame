...
//#define GRID_CELL_SIZE 0.2f  // Размер ячейки сетки
//#define GRID_WIDTH ((int)(2.0f / GRID_CELL_SIZE) + 1)  // [-1, 1] -> ширина сетки
//#define GRID_HEIGHT ((int)(2.0f / GRID_CELL_SIZE) + 1) // [-1, 1] -> высота сетки
//
//typedef struct {
//    int bulletIndices[MAX_BULLETS];
//    int bulletCount;
//    int enemyIndices[MAX_ENEMIES];
//    int enemyCount;
//    int collisionChecks;  // Количество проверок коллизий в этой ячейке
//} GridCell;
//
//GridCell spatialGrid[GRID_WIDTH][GRID_HEIGHT];
//
...
//    // Метрики рендеринга
//    int drawCalls;
//    int drawCallsBullets;
//    int drawCallsEnemies;
//    int drawCallsPlayer;
//    int drawCallsBackground;
//    int drawCallsText;
//
//    // Метрики коллизий
//    long long collisionChecksTotal;
//    long long collisionChecksWithGrid;
//    long long collisionChecksWithoutGrid;
//    int collisionHits;
//
//    // Метрики объектов
//    int activeEnemies;
...
//    float avgObjectsPerCell;
//
//    // Статистика оптимизации
//    double collisionTimeMs;
//    double gridBuildTimeMs;
//} PerformanceMetrics;
//
//PerformanceMetrics metrics = { 0 };
//long long totalFrames = 0;
//
//// ========== МАКРОСЫ ДЛЯ СБОРА МЕТРИК ==========
//#define START_PROFILE_TIMER(var) double var##_start = glfwGetTime()
//#define END_PROFILE_TIMER(var) metrics.var += (glfwGetTime() - var##_start) * 1000.0
//
//#define INCREMENT_DRAW_CALLS() metrics.drawCalls++
//#define INCREMENT_COLLISION_CHECKS() metrics.collisionChecksTotal++
//#define INCREMENT_COLLISION_HITS() metrics.collisionHits++
//
//// ============================================
//
//// === Шейдеры ===
...
//unsigned int VAO_b, VBO_b, bulletInstanceVBO;
//
//unsigned int enemyShader;
//unsigned int enemyVAO, enemyVBO, enemyInstanceVBO;
//Model enemymodel;
//
//// Для игрока
...
//Model playermodel;
//
//// ========== ФУНКЦИИ ПРОСТРАНСТВЕННОЙ СЕТКИ ==========
//
//// Очистка сетки
//void clearSpatialGrid() {
//    for (int i = 0; i < GRID_WIDTH; i++) {
//        for (int j = 0; j < GRID_HEIGHT; j++) {
//            spatialGrid[i][j].bulletCount = 0;
//            spatialGrid[i][j].enemyCount = 0;
//            spatialGrid[i][j].collisionChecks = 0;
//        }
//    }
//}
//
//// Преобразование координаты в индекс сетки
//int worldToGridCoord(float worldCoord) {
//    // Преобразуем из [-1, 1] в [0, GRID_SIZE-1]
//    return (int)((worldCoord + 1.0f) / GRID_CELL_SIZE);
//}
//
//// Добавление пули в сетку
//void addBulletToGrid(int bulletIndex) {
//    if (!bullets[bulletIndex].active) return;
//
//    int gridX = worldToGridCoord(bullets[bulletIndex].x);
//    int gridY = worldToGridCoord(bullets[bulletIndex].y);
//
//    if (gridX >= 0 && gridX < GRID_WIDTH && gridY >= 0 && gridY < GRID_HEIGHT) {
//        GridCell* cell = &spatialGrid[gridX][gridY];
//        if (cell->bulletCount < MAX_BULLETS) {
//            cell->bulletIndices[cell->bulletCount++] = bulletIndex;
//        }
//    }
//}
//
...
//
//    int gridX = worldToGridCoord(enemies[enemyIndex].x);
//    int gridY = worldToGridCoord(enemies[enemyIndex].y);
//
//    if (gridX >= 0 && gridX < GRID_WIDTH && gridY >= 0 && gridY < GRID_HEIGHT) {
//        GridCell* cell = &spatialGrid[gridX][gridY];
//        if (cell->enemyCount < MAX_ENEMIES) {
//            cell->enemyIndices[cell->enemyCount++] = enemyIndex;
//        }
//    }
//}
//
//// Построение пространственной сетки (вызывать каждый кадр)
//void buildSpatialGrid() {
//    START_PROFILE_TIMER(gridBuild);
//
//    clearSpatialGrid();
//
//    // Добавляем все активные пули
...
//    }
//
//    // Собираем статистику по сетке
//    metrics.gridCellsUsed = 0;
//    int totalObjects = 0;
//    metrics.maxObjectsPerCell = 0;
//
//    for (int i = 0; i < GRID_WIDTH; i++) {
//        for (int j = 0; j < GRID_HEIGHT; j++) {
//            GridCell* cell = &spatialGrid[i][j];
//            int cellObjects = cell->bulletCount + cell->enemyCount;
//            if (cellObjects > 0) {
//                metrics.gridCellsUsed++;
//                totalObjects += cellObjects;
//                if (cellObjects > metrics.maxObjectsPerCell) {
//                    metrics.maxObjectsPerCell = cellObjects;
//                }
//            }
//        }
//    }
//
//    metrics.avgObjectsPerCell = metrics.gridCellsUsed > 0 ?
//        (float)totalObjects / metrics.gridCellsUsed : 0.0f;
//
//    END_PROFILE_TIMER(gridBuild);
//}
//
//// Проверка столкновений БЕЗ сетки (для сравнения)
//void checkCollisionsWithoutGrid() {
//    START_PROFILE_TIMER(collision);
//
...
//
//            INCREMENT_COLLISION_CHECKS();
//            metrics.collisionChecksWithoutGrid++;
//
//            if ((fabs(bullets[i].x - enemies[j].x) <= ENEMY_SIZEX) &&
//                (fabs(bullets[i].y - enemies[j].y) <= ENEMY_SIZEY)) {
//
//                INCREMENT_COLLISION_HITS();
//                enemies[j].lives--;
//                bullets[i].active = 0;
//                bulletCount--;
//                enemies[j].hit = 1;
...
//    }
//
//    END_PROFILE_TIMER(collision);
//}
//
//// Проверка столкновений С ИСПОЛЬЗОВАНИЕМ сетки
//void checkCollisionsWithGrid() {
//    START_PROFILE_TIMER(collision);
//
...
//            GridCell* cell = &spatialGrid[gridX][gridY];
//
//            // Проверяем столкновения между пулями и врагами в этой ячейке
//            for (int bi = 0; bi < cell->bulletCount; bi++) {
//                int bulletIdx = cell->bulletIndices[bi];
//
//                // Проверяем только пули игрока (dir = 1)
//                if (!bullets[bulletIdx].active || bullets[bulletIdx].dir != 1)
//                    continue;
//
//                for (int ei = 0; ei < cell->enemyCount; ei++) {
//                    int enemyIdx = cell->enemyIndices[ei];
//
//                    if (!enemies[enemyIdx].active)
//                        continue;
//
//                    INCREMENT_COLLISION_CHECKS();
//                    metrics.collisionChecksWithGrid++;
//                    cell->collisionChecks++;
//
//                    // Быстрая проверка AABB
//                    float dx = fabsf(bullets[bulletIdx].x - enemies[enemyIdx].x);
//                    float dy = fabsf(bullets[bulletIdx].y - enemies[enemyIdx].y);
//
//                    if (dx <= ENEMY_SIZEX && dy <= ENEMY_SIZEY) {
//                        // Столкновение обнаружено
//                        INCREMENT_COLLISION_HITS();
//                        enemies[enemyIdx].lives--;
//                        bullets[bulletIdx].active = 0;
//                        bulletCount--;
//                        enemies[enemyIdx].hit = 1;
//
//                        if (enemies[enemyIdx].lives == 0) {
//                            enemies[enemyIdx].active = 0;
//                            kills++;
//                            score += baseEnemyScore + 5 * wave;
//                            metrics.enemiesKilled++;
//                        }
//
//                        // Пуля уничтожена, выходим из внутреннего цикла
//                        break;
//                    }
//                }
//            }
...
//{
//    static float offsets[MAX_BULLETS * 3];
//    int instanceCount = 0;
//
//    for (int i = 0; i < MAX_BULLETS; i++) {
//        if (bullets[i].active) {
...
//
//    glBindBuffer(GL_ARRAY_BUFFER, bulletInstanceVBO);
//    glBufferSubData(GL_ARRAY_BUFFER, 0, instanceCount * 3 * sizeof(float), offsets);
//
...
//void updateEnemy()
//{
//    // Используем пространственную сетку для проверки столкновений
//    buildSpatialGrid();
//    checkCollisionsWithGrid();
//
//    // Подсчитываем активные объекты
//    metrics.activeEnemies = 0;
//    metrics.activeBullets = 0;
//
//    for (int i = 0; i < MAX_ENEMIES; i++) {
//        if (enemies[i].active) metrics.activeEnemies++;
//    }
//
//    for (int i = 0; i < MAX_BULLETS; i++) {
//        if (bullets[i].active) metrics.activeBullets++;
//    }
//}
//
//void spawnFormation()
...
//{
//    static char instanceData[MAX_ENEMIES * 16]; // 3*float + 1*int = 16 байт
//    int instanceCount = 0;
//
//    for (int i = 0; i < MAX_ENEMIES; i++) {
...
//    if (instanceCount == 0) return;
//
//    glBindBuffer(GL_ARRAY_BUFFER, enemyInstanceVBO);
//    glBufferSubData(GL_ARRAY_BUFFER, 0, instanceCount * 16, instanceData);
//
...
//}
//
//// ========== ФУНКЦИИ ДЛЯ ОТОБРАЖЕНИЯ МЕТРИК ==========
//
//void renderPerformanceMetrics() {
//    char buffer[256];
//
//    // Основные метрики
//    sprintf(buffer, "FPS: %.1f | Frame: %.2fms", metrics.fps, metrics.frameTime);
//    renderText(buffer, -0.95f, 0.8f, 0.6f);
//
//    // Метрики рендеринга
//    sprintf(buffer, "DrawCalls: %d (B:%d E:%d P:%d T:%d)",
//        metrics.drawCalls,
//        metrics.drawCallsBullets,
//        metrics.drawCallsEnemies,
//        metrics.drawCallsPlayer,
//        metrics.drawCallsText);
//    renderText(buffer, -0.95f, 0.7f, 0.5f);
//
//    // Метрики коллизий
//    float hitRate = metrics.collisionChecksTotal > 0 ?
//        (float)metrics.collisionHits / metrics.collisionChecksTotal * 100.0f : 0.0f;
//
//    sprintf(buffer, "Collisions: %lld checks, %d hits (%.1f%%)",
//        metrics.collisionChecksTotal, metrics.collisionHits, hitRate);
//    renderText(buffer, -0.95f, 0.6f, 0.5f);
//
//    // Метрики сетки
//    if (metrics.collisionChecksWithGrid > 0) {
//        long long savedChecks = metrics.collisionChecksWithoutGrid - metrics.collisionChecksWithGrid;
//        float savings = metrics.collisionChecksWithoutGrid > 0 ?
//            (float)savedChecks / metrics.collisionChecksWithoutGrid * 100.0f : 0.0f;
//
//        sprintf(buffer, "Grid: %d/%d cells, %.1f obj/cell",
//            metrics.gridCellsUsed, GRID_WIDTH * GRID_HEIGHT, metrics.avgObjectsPerCell);
//        renderText(buffer, -0.95f, 0.5f, 0.5f);
//
//        sprintf(buffer, "Checks saved: %lld (%.1f%%)", savedChecks, savings);
//        renderText(buffer, -0.95f, 0.4f, 0.5f);
//    }
//
//    // Метрики объектов
//    sprintf(buffer, "Objects: %d enemies, %d bullets",
//        metrics.activeEnemies, metrics.activeBullets);
//    renderText(buffer, -0.95f, 0.3f, 0.5f);
//
//    // Время обработки
//    sprintf(buffer, "Times: Collision: %.2fms, Grid: %.2fms",
//        metrics.collisionTimeMs, metrics.gridBuildTimeMs);
//    renderText(buffer, -0.95f, 0.2f, 0.5f);
//}
//
//void resetMetricsForFrame() {
//    metrics.drawCalls = 0;
//    metrics.drawCallsBullets = 0;
//    metrics.drawCallsEnemies = 0;
//    metrics.drawCallsPlayer = 0;
//    metrics.drawCallsBackground = 0;
//    metrics.drawCallsText = 0;
//
//    metrics.collisionChecksTotal = 0;
//    metrics.collisionChecksWithGrid = 0;
//    metrics.collisionChecksWithoutGrid = 0;
//    metrics.collisionHits = 0;
//
//    metrics.collisionTimeMs = 0;
//    metrics.gridBuildTimeMs = 0;
//}
//
//// ===================================================
//
...
//    glGenBuffers(1, &bulletInstanceVBO);
//    glBindVertexArray(VAO_b);
//
...
//
//    glBindBuffer(GL_ARRAY_BUFFER, bulletInstanceVBO);
//    glBufferData(GL_ARRAY_BUFFER, MAX_BULLETS * 3 * sizeof(float), NULL, GL_DYNAMIC_DRAW);
...
//    glEnableVertexAttribArray(2);
//
//    glGenBuffers(1, &enemyInstanceVBO);
//    glBindBuffer(GL_ARRAY_BUFFER, enemyInstanceVBO);
//    glBufferData(GL_ARRAY_BUFFER, MAX_ENEMIES * 16, NULL, GL_DYNAMIC_DRAW);
//
//    glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 16, (void*)0);
//    glEnableVertexAttribArray(3);
//    glVertexAttribDivisor(3, 1);
//
//    glVertexAttribPointer(4, 1, GL_INT, GL_FALSE, 16, (void*)(12));
...
//#if PROFILING_MODE
//    profileLog = fopen("profile_metrics.txt", "w");
//    if (profileLog) {
//        fprintf(profileLog, "Frame,Time_sec,FPS,FrameTime_ms,DrawCalls,BulletDrawCalls,EnemyDrawCalls,"
//            "PlayerDrawCalls,TextDrawCalls,CollisionChecksTotal,CollisionChecksWithGrid,"
//            "CollisionChecksWithoutGrid,CollisionHits,ActiveEnemies,ActiveBullets,"
//            "GridCellsUsed,AvgObjectsPerCell,MaxObjectsPerCell,CollisionTimeMs,GridBuildTimeMs\n");
//        fflush(profileLog);
//    }
//#endif
//
...
//            metrics.fps = fpsFrames / (currentTime - fpsUpdateTime);
//            fpsUpdateTime = currentTime;
//            fpsFrames = 0;
//        }
//        metrics.frameTime = deltaTime * 1000.0;
//
//        // Сбрасываем метрики для нового кадра
//        resetMetricsForFrame();
//        totalFrames++;
//
//#if !PROFILING_MODE
...
//        case STATE_PLAYING:
//#if !PROFILING_MODE
//            updateEnemy(); // Теперь использует пространственную сетку
//            updateBullets();
//            for (int j = 0; j < MAX_ENEMIES; j++)
...
//
//            // Отображаем метрики производительности
//            if (PROFILING_MODE) {
...
//#if PROFILING_MODE
//        if (profileLog) {
//            fprintf(profileLog, "%lld,%.3f,%.2f,%.3f,%d,%d,%d,%d,%d,%lld,%lld,%lld,%d,%d,%d,%d,%.2f,%d,%.3f,%.3f\n",
//                totalFrames,
//                currentTime,
//                metrics.fps,
//                metrics.frameTime,
...
//                metrics.drawCallsPlayer,
//                metrics.drawCallsText,
//                metrics.collisionChecksTotal,
//                metrics.collisionChecksWithGrid,
//                metrics.collisionChecksWithoutGrid,
//                metrics.collisionHits,
//                metrics.activeEnemies,
//                metrics.activeBullets,
//                metrics.gridCellsUsed,
//                metrics.avgObjectsPerCell,
//                metrics.maxObjectsPerCell,
//                metrics.collisionTimeMs,
//                metrics.gridBuildTimeMs);
//            fflush(profileLog);
//        }
//#endif
//
...
//        fprintf(profileLog, "\n\n=== SUMMARY ===\n");
//        fprintf(profileLog, "Total frames: %lld\n", totalFrames);
//        fprintf(profileLog, "Average FPS: %.2f\n", totalFrames / (glfwGetTime()));
//        fprintf(profileLog, "Average DrawCalls per frame: %.1f\n",
//            (float)metrics.drawCalls / totalFrames);
//        fprintf(profileLog, "Average collision checks per frame: %.1f\n",
//            (float)metrics.collisionChecksTotal / totalFrames);
//        fprintf(profileLog, "Collision hit rate: %.2f%%\n",
//            (float)metrics.collisionHits / metrics.collisionChecksTotal * 100.0f);
//        fclose(profileLog);
//    }
//#endif
//
